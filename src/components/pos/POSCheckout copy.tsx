...
export default function POSCheckout() {
  ...
  useEffect(() => {
    ...
    if (settings?.currency && settings?.locale) {
      setItemTotalFormatted(formatCurrencyNumber(subtotal, settings.currency as string, settings.locale as string));
      setFinalTotalFormatted(formatCurrencyNumber(netTotal, settings.currency as string, settings.locale as string));
    }
  }, [cartData, settings]);
  ...
  async function createPOSOrder() {
    try {
      ...
      router.push(`/complete?paymentType=${paymentType}&orderMasterId=${orderId}&deliveryType=pickup`);
    } catch (err) {
      console.error(err);
    } finally {
      setIsLoading(false);
